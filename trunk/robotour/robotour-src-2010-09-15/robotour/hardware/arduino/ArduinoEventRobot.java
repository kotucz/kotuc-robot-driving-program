...
public class ArduinoEventRobot implements EventListener {
    ...
    SonarDensity density = new SonarDensity(odometry);
...
//        iframe.setImage(birdsEye.getFloorImage());
    ...
    Exploring exploring = new Exploring(birdsEye.getFloor(), density);
...
//        iframe.setImage(birdsEye.getFloorImage());
            ...
            public void run() {
                while (true) {
                    try {
                        ...
                        double speed = 0.4;
                        double steer = exploring.randomSearch(odometry, speed, null);
    ...
    void startVideoSamplingThread() {
        new Thread() {
            ...
            @Override
            public void run() {
                VideoEventRecorder videoRecorder = new VideoEventRecorder(VideoInputs.getVideo(), eventLog);
                while (true) {
                    try {
