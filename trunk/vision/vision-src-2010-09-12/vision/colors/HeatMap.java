...
//        this.src = src;
     ...
     * @return
     */
    public static HeatMap createHeatMap(BufferedImage img, HeatFunction func) {
        return new HeatMap(img, func);
    }

    public BufferedImage createRoadBnWImage() {
...
//    }
    ...
    public static final HeatVizualizer RED_GREEN = new HeatVizualizer() {

        public int color(double prob) {

            int green = (int) Math.round(255 * prob);
            int red = 255 - green;
            return 0xFF000000 + (red << 8) + (green << 16);
//
//            if (prob > 500) {
//                return 0xFf0000FF;
//            } else if (prob > 200) {
...
//            }
    ...
    public static final HeatVizualizer RED_GREEN_TESH(final double tresh) {
        return new HeatVizualizer() {

            public int color(double prob) {
                if (prob < tresh) {
                    return 0xFF000000 + (int) Math.round(255 * prob) << 8;
                } else {
                    return 0xFF000000 + (int) Math.round(255 * prob) << 16;
                }
            }
        };
    }
    ...
    public static final HeatVizualizer THERMAL = new HeatVizualizer() {
...
//        System.out.println("pix "+pix+" dot "+pix.dot(colorrvector)+" length "+pix.length());
    ...
    static class HeatMapCM implements HeatFunction {
        ...
        private ColorHistogram histogram = new ColorHistogram();
//    static Histogram histogram = new Histogram(new File("C:/Users/Kotuc/Desktop/snap000740.png"));
    ...
    /**
     * Preffered function to boolean value of road occurence.
     * @param x
     * @param y
     * @return
     */
    public boolean isRoad(int x, int y) {
        return getHeat(x, y) > treshold;
    }
...
//                    heatMap.setRGB(x, y, 0xFF0000);
    ...
    public static final HeatFunction SATURATION_HEAT_FUNCTION = new HeatFunction() {

        public float heat(int rgb) {
            float[] hsb = new float[3];
            Color.RGBtoHSB(Colors.getRed(rgb), Colors.getGreen(rgb), Colors.getBlue(rgb), hsb);
            return 1 - hsb[1];
        }
    };
    public static final HeatFunction HUE_HEAT_FUNCTION = new HeatFunction() {
