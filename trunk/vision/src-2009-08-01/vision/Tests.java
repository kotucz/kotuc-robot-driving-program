...
import vision.pathfinding.OldWomen;
import vision.pathfinding.HeatMap;
import vision.pathfinding.PathFinding;
...
class Tests {
    ...
    static BufferedImage colorMap(BufferedImage input) {
        StColorMap.load(input);
        return new StColorMap().getTable();
    }

    static BufferedImage pathFinding(BufferedImage input) {
        return new StColorMap().getTable();
    }
...
//        return objectAnalysis.prepairImage(bi);
    ...
    static BufferedImage showWay(BufferedImage input) {

        BufferedImage img = BasicOps.copy(input);
        float uhel = PathFinding.paintCrossRoad2(img, 0);
        return img;
    }
    ...
    static BufferedImage groundWoman4(BufferedImage input) {
        return OldWomen.groundWoman4(input);
    }
...
//                Color3f cv = new Color3f(v3);
    ...
    static BufferedImage getFinal(BufferedImage input) {
        ...
        for (float steer : steers) //        BufferedImage mask = masks.get(maskid);
        {
            ...
            double dist = PathFinding.paintTrailsCM(bi, steer);
...
//        }
    ...
    static BufferedImage precog(BufferedImage input) {
        precog.process(BasicOps.getResizedImage(input, 64, 48));
        return precog.getResult();
    }

    static BufferedImage toHSB(BufferedImage input) {
        return HSBImage.rgbToHSB(input);
    }
...
//        augPaint.setPos(new Point3d(0, 0, -1));
    ...
    static BufferedImage camCalibration(BufferedImage input) {
        ...
        Point p = CameraProportions.getCameraProportions().getProjectionImagePoint(new Point3d(0.5, 0.5, -2.0));
