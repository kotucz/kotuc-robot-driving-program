...
//        if (Math.abs(pattern.gray() - sample.gray()) > 20) {
//            // reject;
//            System.out.println("reject");
//            return (pattern.gray() - sample.gray()) / 255.0;
//        }
...
//                    diff++;
//                }
                diff += Math.abs((im1.getRGB(x, y) & MASK) - (im2.getRGB(x, y) & MASK));

                //diff += Colors.dist(pattern.getRGB(x, y), sample.getRGB(x, y));
            }
        }

        return 1 - ((double) diff / (MASK * iw * ih));
