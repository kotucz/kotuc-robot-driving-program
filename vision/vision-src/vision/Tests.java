...
import vision.colors.HSBConvertor;
...
import vision.colors.HSHistogramOld;
...
import vision.pathfinding.knowledge.KnownRecog2;
...
//        }
    ...
    static BufferedImage pathRecog(BufferedImage input) {
        precog.process(input);
        return precog.getResult();
    }
    private static CharacteristicsRecog chrecog = new CharacteristicsRecog();

    static BufferedImage charRecog(BufferedImage input) {
        chrecog.process(input);
        return chrecog.getDebugImage();
    }
//    private static KnownRecog knownrecog = new KnownRecog();
//    static {
//        knownrecog.createKnowledge(new File("../robotour/knowledge"));
//    }

    static BufferedImage knownRecog(BufferedImage input) {
        KnownRecog knownrecog = new KnownRecog();
        knownrecog.createKnowledge(new File("../robotour/knowledge"));
        knownrecog.process(input);
        return knownrecog.getResult();
    }

    static BufferedImage knownRecog2(BufferedImage input) {
        KnownRecog2 knownrecog = new KnownRecog2();
        knownrecog.createKnowledge(new File("../robotour/knowledge"));
        knownrecog.process(input);
        return knownrecog.getResult();
    }

    static BufferedImage hue(BufferedImage input) {
        return HSBConvertor.hue(input);
    }

    static BufferedImage saturation(BufferedImage input) {
        return HSBConvertor.saturation(input);
    }

    static BufferedImage hsTable(BufferedImage input) {
        return new HSHistogramOld().parse(input);
    }

    static BufferedImage hsRoad(BufferedImage input) {
        return HSHistogramOld.roadTable();
    }
...
//    }
    ...
    static BufferedImage planeCanvas(BufferedImage image) {
        PlaneCanvas canvas = new PlaneCanvas();
        canvas.getPlaneProjection().setCamera(new Point3d(0, 0.16, 0.35), 0, Math.PI / 3);
//        canvas.getPlaneProjection().setCamera(new Point3d(-10, 0.16, 13), 0, Math.PI/4);
        canvas.setGraphics(image.getGraphics());
        canvas.drawGrid(0, 0);
        return image;
    }

    static BufferedImage patternPlaneCanvas(BufferedImage image) {
        PlaneCanvas canvas = new PlaneCanvas();
        image = aps.paintWireCubeOnSquarePerspective(image);
        canvas.getPlaneProjection().setPlaneTransform(aps.getTrans());
//        canvas.getPlaneProjection().setCamera(new Point3d(-10, 0.16, 13), 0, Math.PI/4);
        canvas.setGraphics(image.getGraphics());
        canvas.drawGrid(0, 0);
        return image;
    ...
    static BufferedImage birdsEye(BufferedImage image) {
        BirdsEye birdsEye = new BirdsEye(new RealFloor());
        birdsEye.setCameraPosition(0, 0, Math.toRadians(15));
        birdsEye.paintProjectedCameraImage(image);
        return birdsEye.getFloorImage();
    }
